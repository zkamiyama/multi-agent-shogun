...
mux_zellij_write_web_config() {
    local web_sharing="$1"
    case "$web_sharing" in
        on|off|disabled) ;;
        *)
            echo "[mux/zellij] invalid SHOGUN_ZELLIJ_WEB_SHARING: ${web_sharing}" >&2
            return 12
            ;;
    esac

    local config_path="${SHOGUN_ZELLIJ_WEB_CONFIG:-$(dirname "$MUX_STATE_FILE")/zellij-webshare.kdl}"
    local config_dir
    config_dir="$(dirname "$config_path")"
    mkdir -p "$config_dir"

    local source_config="${ZELLIJ_CONFIG_FILE:-}"
    if [ -z "$source_config" ]; then
        if [ -n "${ZELLIJ_CONFIG_DIR:-}" ]; then
            source_config="${ZELLIJ_CONFIG_DIR}/config.kdl"
        else
            source_config="${HOME}/.config/zellij/config.kdl"
        fi
    fi

    local tmp="${config_path}.tmp.$$"
    if [ -r "$source_config" ] && [ "$source_config" != "$config_path" ]; then
    ...
    {
        printf '\n// Generated by multi-agent-shogun for browser-access session creation.\n'
        printf 'web_sharing "%s"\n' "$web_sharing"
        printf 'web_server false\n'
    } >> "$tmp"
    mv "$tmp" "$config_path"
    printf '%s\n' "$config_path"
}
...
mux_backend_delete_session() {
    local session="$1"
    mux_backend_preflight || return $?
    if ! mux_backend_has_session "$session"; then
        return 1
    fi
    mux_zellij_cmd delete-session --force "$session" >/dev/null || return $?
    ...
    echo "[mux/zellij] session still exists after delete-session --force: ${session}" >&2
    return 12
}
