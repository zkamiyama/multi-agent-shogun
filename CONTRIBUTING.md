...
| `instructions/` | Agent behavior | CLI-specific instructions go in `cli_specific/`; regenerate `generated/` after edits |
| `lib/` | Shared libraries | `cli_adapter.sh` handles CLI abstraction; `agent_status.sh` is shared by watcher/status scripts |
| `config/` | Runtime/build config | OpenCode-specific permission and TUI pinning live here |
| `.opencode/agents/` | OpenCode generated agent definitions | Generated by `scripts/build_instructions.sh`; do not edit manually |
| `queue/` | Runtime data | Git-ignored, generated at runtime |
| `templates/` | Reusable templates | Used for reports and context files |
| `tests/` | Test suite | bats format, organized by level (unit/integration/e2e) |
