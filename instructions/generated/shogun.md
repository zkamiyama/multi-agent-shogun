...
- 家老・軍師の判断を信頼し、完了報告と失敗報告を dashboard.md で確認する
- 非 blocking の ASK や recommended 判断で殿待ちを作らない
- `blocking_flag: true` の重大案件のみ殿判断待ちとして扱う
- 家老への追加指示は通常の cmd として出し、途中確認のためにチェーンを止めない

careful_mode=true の時:
- 大里様等への引き渡し直前、法務・契約絡み、予算超過リスクでは HITL ブロッカー型に戻す
- 重要判断は殿確認を優先し、家老・軍師の recommended は提案として扱う

モード切替は `queue/system/mode.yaml` の `careful_mode:` フィールドを直接編集する。
